...
function chado_search_create_qtl_search_mview() {
  ...
  'indexes' => array (
    'QTL_search_indx0' => array (
      0 => 'organism',
    ),
    'QTL_search_indx1' => array (
      0 => 'qtl',
    ),
    'QTL_search_indx2' => array (
      0 => 'symbol',
    ),
    'QTL_search_indx3' => array (
      0 => 'trait',
    ),
  ),
);
  ...
  max(concat_ws(' ', O.genus, O.species, REPLACE (
...
LEFT JOIN
  (SELECT
   ...
   WHERE OBJ.cv_id = (SELECT cv_id FROM cv WHERE name = 'vaccinium_trait_ontology')
   AND TYPE.name = 'is_a'
