...
function chado_search_create_marker_search_mview() {
  ...
  --- Get genome location
      ...
      AND F2.type_id IN (SELECT cvterm_id FROM cvterm WHERE name IN ('genetic_marker', 'QTL', 'heritable_phenotypic_marker') AND cv_id = (SELECT cv_id FROM cv WHERE name = 'sequence'))
